import 'package:flutter/material.dart';
import 'login_screen.dart';

class SchoolManagementHomeScreen extends StatefulWidget {
  const SchoolManagementHomeScreen({super.key});

  @override
  _SchoolManagementHomeScreenState createState() =>
      _SchoolManagementHomeScreenState();
}

class _SchoolManagementHomeScreenState
    extends State<SchoolManagementHomeScreen> {
  @override
  Widget build(BuildContext context) {
    final currentWidth = MediaQuery.of(context).size.width;

    return Scaffold(
      backgroundColor: const Color(0xFFF5EEDC),
      body: Padding(
        padding: const EdgeInsets.all(8.0),
        child: SafeArea(
          child: SingleChildScrollView(
            child: Column(
              children: [
                Align(
                  alignment: Alignment.centerLeft,
                  child: IconButton(
                    icon: const Icon(Icons.arrow_back, color: Colors.black),
                    onPressed: () {
                      Navigator.pushReplacement(
                        context,
                        MaterialPageRoute(builder: (context) => LoginScreen()),
                      );
                    },
                  ),
                ),
                const SizedBox(height: 20),
                Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    _buildSchoolInfo(),
                    const SizedBox(height: 20),
                    _buildLabel('Instant Messaging'),
                    const SizedBox(height: 10),
                    _buildLabel('Control panel'),
                    const SizedBox(height: 12),
                    _buildControlPanel(currentWidth),
                  ],
                ),
              ],
            ),
          ),
        ),
      ),
    );
  }

  Widget _buildLabel(String label) {
    return Container(
      padding: const EdgeInsets.all(8),
      decoration: BoxDecoration(
        color: const Color(0xFFFBFBED),
        borderRadius: BorderRadius.circular(8),
      ),
      child: Text(
        label,
        style: const TextStyle(
          fontWeight: FontWeight.bold,
          color: Color(0xFF3B7962),
        ),
      ),
    );
  }

  Widget _buildSchoolInfo() {
    return Container(
      decoration: BoxDecoration(borderRadius: BorderRadius.circular(12)),
      child: Card(
        elevation: 0.2,
        shadowColor: Colors.black,
        color: const Color(0xFFFFF8F0),
        child: Padding(
          padding: const EdgeInsets.all(16),
          child: Row(
            mainAxisAlignment: MainAxisAlignment.center,
            crossAxisAlignment: CrossAxisAlignment.center,
            children: [
              CircleAvatar(
                radius: 40,
                backgroundColor: const Color(0xFF3B7962),
                child: Image.asset(
                  "assets/images/school.png",
                  width: 96,
                  height: 96,
                  fit: BoxFit.cover,
                ),
              ),
              const SizedBox(width: 8),
              Expanded(
                child: LayoutBuilder(
                  builder: (context, constraints) {
                    double fontSize = constraints.maxWidth * 0.05;
                    return Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      mainAxisAlignment: MainAxisAlignment.center,
                      children: [
                        _responsiveText('School name: ..........', fontSize),
                        _responsiveText('School address: ..........', fontSize),
                        _responsiveText(
                          'Main contact Number: ....................',
                          fontSize,
                        ),
                        _responsiveText(
                          'E-mail: ....................',
                          fontSize,
                        ),
                      ],
                    );
                  },
                ),
              ),
              const Icon(Icons.edit, color: Colors.black54),
            ],
          ),
        ),
      ),
    );
  }

  Widget _buildControlPanel(double fullWidth) {
    final width = (fullWidth / 3) - (fullWidth / 20);
    return Column(
      children: [
        Row(
          mainAxisAlignment: MainAxisAlignment.spaceEvenly,
          children: [
            _buildControlButton('assets/images/buses.png', 'Buses', width),
            _buildControlButton(
              'assets/images/students.png',
              'Students',
              width,
              isStudents: true,
            ),
            _buildControlButton(
              'assets/images/busTracking.png',
              'Bus tracking',
              width,
            ),
          ],
        ),
        const SizedBox(height: 8),
        Row(
          mainAxisAlignment: MainAxisAlignment.spaceEvenly,
          children: [
            _buildControlButton(
              'assets/images/report.png',
              'Performance reports',
              width,
            ),
            _buildControlButton(
              'assets/images/admin.png',
              'Admin settings',
              width,
            ),
            _buildControlButton(
              'assets/images/camera.png',
              'Recordings',
              width,
            ),
          ],
        ),
      ],
    );
  }

  Widget _buildControlButton(
    String imagePath,
    String label,
    double cardWidth, {
    bool isStudents = false,
  }) {
    double imageSize = isStudents ? 80 : 66;

    return InkWell(
      onTap: () {
        print('Tapped on $label');
      },
      child: SizedBox(
        width: cardWidth,
        child: Card(
          elevation: 0.2,
          color: Colors.white,
          shape: RoundedRectangleBorder(
            borderRadius: BorderRadius.circular(24),
          ),
          child: Padding(
            padding: const EdgeInsets.all(8.0),
            child: Column(
              mainAxisAlignment: MainAxisAlignment.center,
              children: [
                Image.asset(
                  imagePath,
                  width: imageSize,
                  height: imageSize,
                  fit: BoxFit.contain,
                ),
                const SizedBox(height: 4),
                LayoutBuilder(
                  builder: (context, constraints) {
                    double fontSize = constraints.maxWidth * 0.13;
                    return _responsiveText(
                      label,
                      fontSize,
                      maxLines: 2,
                      textAlign: TextAlign.center,
                    );
                  },
                ),
              ],
            ),
          ),
        ),
      ),
    );
  }

  Widget _responsiveText(
    String text,
    double fontSize, {
    TextAlign textAlign = TextAlign.left,
    int maxLines = 1,
  }) {
    return Text(
      text,
      textAlign: textAlign,
      style: TextStyle(
        color: const Color(0xFF3B7962),
        fontSize: fontSize.clamp(10, 14),
      ),
      overflow: TextOverflow.ellipsis,
      maxLines: maxLines,
    );
  }
}
